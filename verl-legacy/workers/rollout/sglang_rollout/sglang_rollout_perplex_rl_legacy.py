...
class SearcherReaderStateEnum(str, Enum):
...
class SGLangRollout(BaseRollout):
    def __init__(
        ...
        **kwargs,
        ...
        ## Get Perplex-rl Config and initialize LLMGenerator
        config_parser = configparser.ConfigParser()
        config_parser.read("verl/third_party/perplex_rl/config.ini")
        
        self.use_api = self.config.get("use_api", False)

        self.api_planning = self.config.get("api_planning", False)
        self.api_query_rewrite = self.config.get("api_query_rewrite", False)
        self.api_reader = self.config.get("api_reader", False)
        if self.use_api:
            # 3 types of LLM generation: Query Planning (Initial Prompt / API), Query Rewrite (Interaction Prompt / LLM), Answer Generation (Interaction Prompt / API)
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        # Perplex-rl variables:
        searcher_state = SearcherReaderStateEnum.PLANNING # always start from Planning for search-r1, s3, Perplex-rl
            ...
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    if len(_req.input_ids) >= self.config.max_model_len:
                        ...
                        break
                    _req.state = AsyncRolloutRequestStateEnum.RUNNING
                    searcher_state = SearcherReaderStateEnum.PLANNING
                else:
                ...
                ### Perplex-rl TODO: add mechanism to separate query rewrite and planning
                if (self.api_planning and searcher_state == SearcherReaderStateEnum.PLANNING) or (self.api_query_rewrite and searcher_state == SearcherReaderStateEnum.REWRITING):
                    output =  await self._handle_engine_api_generate(_req, request_sampling_params, image_data)
                else:
                    # query rewrite here
                ...
                if self.has_answer(content):
                    searcher_state = SearcherReaderStateEnum.READING
                    # add tokens and log probs to the request
                    print(output)
                    generated_tokens = output['tokens']
                    generated_log_probs = output['log_probs']
                    # put them into interaction instance dict
                    ...
                    interaction._instance_dict[_req.request_id]["tokens"] = generated_tokens
                    interaction._instance_dict[_req.request_id]["log_probs"] = generated_log_probs

                if finish_reason_type == FinishReasonTypeEnum.LENGTH:
                    if (self.api_planning and searcher_state == SearcherReaderStateEnum.PLANNING) or (self.api_query_rewrite and searcher_state == SearcherReaderStateEnum.REWRITING) or (self.api_reader and searcher_state == SearcherReaderStateEnum.READING):
                    ...
                    if self._function_call_parser and self._function_call_parser.has_tool_call(content):
                        ...
                        # Perplex-rl: if we arrive here then it is certain that no tool call if found
                        # Search-r1: api_planning = False, api_reader = False
                        # s3: api_planning = False, api_reader = True
                        # qr: api_planning = True, api_reader = True
                        if self.api_planning and searcher_state == SearcherReaderStateEnum.PLANNING:
                ...
                ## Perplex-rl add: interaction plan detected
                plan_detected = interaction._instance_dict[_req.request_id]["plan_detected"]
                ...
                if should_terminate_sequence:
                    ...
                    if len(_req.input_ids) >= self.config.max_model_len:
                        ...
                        if plan_detected:   
                            searcher_state = SearcherReaderStateEnum.REWRITING
                        _req.state = AsyncRolloutRequestStateEnum.RUNNING
    ...
    async def _handle_engine_call(
        ...
        # print(f"Request messages:{_req.messages}")
    ...
    async def _handle_engine_generate(
        ...
        # print(f'Input:{self.processing_class.decode(generation_prompt_ids)}')
    ...
    async def _handle_rag_engine_api_generate(
        self, 
    ...
    ####### Perplex-RL Implementation #######
    async def _handle_engine_api_generate(
        self, 
        _req: AsyncRolloutRequest, 
        sampling_params: dict, 
        image_data: Optional[list[Any]] = None
        ...
        """
        Handle engine calls using the LLM API.

        Args:
            _req (AsyncRolloutRequest): The rollout request containing the prompt and other metadata.
            sampling_params (dict): Sampling parameters for the LLM API.
            image_data (Optional[list[Any]]): Image data for multi-modal inputs (if applicable).

        Returns:
            dict: The output from the LLM API, including generated text and metadata.
        """
        generation_prompt_ids = _req.get_generation_prompt_ids(self.processing_class)
        ...
        # Step 5: Call the LLM API using the LLMGenerator
        messages = _req.messages
        response = await self.llm_api.generate_response(
            batch_messages=messages,
            **api_sampling_params
        )
        logprobs_obj = response.choices[0].logprobs
        tokens = [t.token for t in logprobs_obj.content]
        logprobs = [t.logprob for t in logprobs_obj.content]
        output_text = response.choices[0].message.content
        # Step 6: Construct the output dictionary
    ...
    def has_plan(self, content)-> bool: 
        # check if <plan> </plan> token
        plan_token = '<plan>'
        end_plan_token = '</plan>'
        return plan_token in content
    
    def has_answer(self, content)-> bool:
        # check if <answer> </answer> token
        answer_token = '<answer>'
        end_answer_token = '</answer>'
        return answer_token in content
