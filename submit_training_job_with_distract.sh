...
TRAINING_SCRIPT="scripts/autograph-r1/run_qwen2.5-7b_instruct_with_distract.sh"
...
CUDA_VISIBLE_DEVICES=$GPU_FOR_EMB nohup bash "$VLLM_EMB_SCRIPT" > vllm_emb_server_distract.log 2>&1 &
VLLM_EMB_PID=$!
echo "vLLM Embedding Server PID: $VLLM_EMB_PID"

# --- Start vLLM Main Server ---
echo "Starting vLLM Main Server..."
CUDA_VISIBLE_DEVICES=$GPU_FOR_MAIN nohup bash "$VLLM_MAIN_SCRIPT" > vllm_main_server_distract.log 2>&1 &
VLLM_MAIN_PID=$!
...
while true; do
    ...
    if grep -q "INFO:     Application startup complete." vllm_emb_server_distract.log; then
        EMB_READY=true
    else
        echo "Still waiting for vLLM Embedding Server to start..."
    fi

    if grep -q "INFO:     Application startup complete." vllm_main_server_distract.log; then
        MAIN_READY=true
    else
...
# --- Extract Server PIDs ---
VLLM_EMB_SERVER_PID=$(grep "INFO:     Started server process" vllm_emb_server_distract.log | awk '{print $NF}' | tr -d '[]')
VLLM_MAIN_SERVER_PID=$(grep "INFO:     Started server process" vllm_main_server_distract.log | awk '{print $NF}' | tr -d '[]')
echo "vLLM Embedding Server Process PID: $VLLM_EMB_SERVER_PID"
echo "vLLM Main Server Process PID: $VLLM_MAIN_SERVER_PID"

# --- Start RAG Server ---
echo "Starting RAG Server..."
nohup python3 "$RAG_SERVER_SCRIPT" > vllm_rag_server_distract.log 2>&1 &
RAG_SERVER_PID=$!
...
while true; do
    if grep -q "INFO:     Application startup complete." vllm_rag_server_distract.log; then
...
RAG_SERVER_PROCESS_PID=$(grep "INFO:     Started server process" vllm_rag_server_distract.log | awk '{print $NF}' | tr -d '[]')
...
while true; do
    ...
    if grep -q "INFO:     Application shutdown complete." vllm_emb_server_distract.log; then
        EMB_SHUTDOWN=true
    else
        echo "Still waiting for vLLM Embedding Server to shutdown..."
    fi

    if grep -q "INFO:     Application shutdown complete." vllm_main_server_distract.log; then
        MAIN_SHUTDOWN=true
    else
        echo "Still waiting for vLLM Main Server to shutdown..."
    fi

    if grep -q "INFO:     Application shutdown complete." vllm_rag_server_distract.log; then
        RAG_SHUTDOWN=true
    else
