#!/bin/bash
#SBATCH --job-name=distract_auto_graph_training_7B
#SBATCH --nodes=1
...
srun --container-writable \
    ...
    bash -c "source /root/miniconda3/etc/profile.d/conda.sh && conda activate verl &&  pip list && cd /workspace/autograph-r1 && bash submit_training_job_with_distract.sh"