...
# --- Configuration ---
VLLM_EMB_SCRIPT="scripts/min_test/vllm_serve/qwen3-0.6b-emb.sh"
VLLM_MAIN_SCRIPT="scripts/min_test/vllm_serve/qwen2.5-7b-vllm.sh"
RAG_SERVER_SCRIPT="autograph-r1/rag_server/rag_server.py"
TRAINING_SCRIPT="scripts/min_test/autograph-r1/run_qwen2.5-7b_instruct_graph_construct.sh"

GPU_FOR_EMB=0 # (set the same 4 in slurm)
GPU_FOR_MAIN=0 # (set the same 4 in slurm)
GPU_FOR_VERL=1 # (set the another 4 in slurm)
...
while true; do
    ...
    if grep -q "INFO:     Application shutdown complete." vllm_emb_server.log; then
        EMB_SHUTDOWN=true
    else
        echo "Still waiting for vLLM Embedding Server to shutdown..."
    fi

    if grep -q "INFO:     Application shutdown complete." vllm_main_server.log; then
        MAIN_SHUTDOWN=true
    else
        echo "Still waiting for vLLM Main Server to shutdown..."
    fi

    if grep -q "INFO:     Application shutdown complete." vllm_rag_server.log; then
        RAG_SHUTDOWN=true
    else
